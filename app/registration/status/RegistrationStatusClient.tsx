    ...
    <div className="min-h-[80vh] px-4 py-16">
      <div className="max-w-2xl mx-auto text-center mb-8">
        ...
        <p className="mt-3 text-[var(--muted-text)]">
          Enter your registration code and email to view the payment
          verification status.
        </p>
      </div>
      ...
      <Card className="max-w-2xl mx-auto">
        <CardHeader>
          ...
          <CardDescription>
            Both fields must match the registration on record.
          </CardDescription>
        </CardHeader>
        <CardContent>
      ...
      {result && (
        <Card className="max-w-2xl mx-auto mt-8">
          <CardHeader>
            <div className="flex items-start justify-between gap-3">
              <div className="flex items-center gap-4 min-w-0">
                <DelegatePhoto
                  url={result.registration.photoUrl}
                  name={result.registration.fullName}
                  size={64}
                />
                <div className="min-w-0">
                  <div className="text-xs uppercase tracking-widest text-[var(--muted-text)]">
              ...
              <Badge
                variant={
                  STATUS_VARIANT[result.registration.status] ?? "secondary"
                }
              >
                {STATUS_LABEL[result.registration.status] ??
          ...
          <CardContent className="space-y-3 text-sm">
            <div>
              <b>Category:</b> {result.registration.category}
            </div>
            <div>
              <b>Fee:</b> ₹
              {result.registration.feeAmount.toLocaleString("en-IN")} (
              {result.registration.feeTier.replace("_", " ")})
            </div>
            <div>
              <b>Submitted:</b>{" "}
              {format(
                new Date(result.registration.createdAt),
                "d MMM yyyy, h:mm a",
              )}
            </div>
            {result.registration.approvedAt && (
              <div>
                <b>Approved:</b>{" "}
                {format(
                  new Date(result.registration.approvedAt),
                  "d MMM yyyy, h:mm a",
                )}
              </div>
            )}
            {result.registration.rejectedAt && (
              <div>
                <b>Rejected:</b>{" "}
                {format(
                  new Date(result.registration.rejectedAt),
                  "d MMM yyyy, h:mm a",
                )}
              </div>
            )}
            {result.registration.reviewNote && (
              <div className="mt-4 p-4 rounded-lg bg-red-50 border-l-4 border-red-500">
                ...
                <div className="whitespace-pre-line text-red-900">
                  {result.registration.reviewNote}
                </div>
                <div className="mt-3">
                  <Link
            ...
            <div className="mt-6 p-4 rounded-lg bg-[var(--surface-100)] border border-[var(--accent-500)]/20">
              <div className="text-xs font-semibold uppercase tracking-wider text-[var(--muted-text)] mb-2">
                Abstract
              </div>
              {result.linkedAbstract ? (
                <div>
                  <div className="font-semibold text-[var(--dark-text)]">
                    {result.linkedAbstract.title}
                  </div>
                  <div className="mt-1 flex items-center gap-2">
                    <span className="text-xs font-mono">
                      {result.linkedAbstract.submissionCode}
                    </span>
                    <Badge variant="outline">
                      {ABS_STATUS_LABEL[result.linkedAbstract.status] ??
                        result.linkedAbstract.status}
                    </Badge>
                  </div>
                </div>
              ) : result.registration.willSubmitAbstract ? (
                <div className="text-sm">
                  You indicated you&apos;d submit an abstract but we
                  haven&apos;t received one yet.
                  <Link
                    href="/abstracts"
                    className="ml-1 font-semibold text-[var(--primary-800)] hover:underline"
                  >
                    Submit now →
                  </Link>
                </div>
