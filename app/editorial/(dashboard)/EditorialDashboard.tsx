...
export default function EditorialDashboard() {
  ...
  const counts = useMemo(() => {
    ...
    abstracts.forEach(
      (a) => (byStatus[a.status] = (byStatus[a.status] ?? 0) + 1),
    );
    return {
      byStatus,
      unassigned: abstracts.filter((a) => a.assignedReviewers.length === 0)
        .length,
    };
  }, [abstracts]);
  ...
  return (
    <div className="p-4 md:p-8">
      <PageHeader
        ...
        description="Route abstracts to reviewers and record final decisions."
      />
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <StatCard
          label="Under review"
          value={counts.byStatus.under_review ?? 0}
          icon={Clock}
        />
        <StatCard
          label="Accepted"
          value={counts.byStatus.accepted ?? 0}
          icon={CheckCircle2}
          accent="emerald"
        />
        <StatCard
          label="Rejected"
          value={counts.byStatus.rejected ?? 0}
          icon={XCircle}
        />
      </div>
      ...
      <Card className="mb-6">
        <CardHeader>
          <CardTitle>Abstracts awaiting reviewer assignment</CardTitle>
        </CardHeader>
              ...
              {loading ? (
                ...
                <TableRow>
                  <TableCell
                    ...
                    className="text-center py-8 text-sm text-[var(--muted-text)]"
                  >
                    Every abstract has reviewers assigned.
                  </TableCell>
                </TableRow>
              ) : (
                needsAssignment.map((a) => (
                  <TableRow key={a._id}>
                    ...
                    <TableCell>
                      <Link
                        href={`/editorial/abstracts/${a._id}`}
      ...
      <div className="mb-4 flex items-start gap-2 p-3 rounded-lg bg-[var(--surface-50)] border border-[var(--accent-500)]/25 text-sm">
        ...
        <span className="text-[var(--muted-text)]">
          Payments are verified by Razorpay and registrations are approved{" "}
          <b>automatically</b> on capture. The list below is checkouts that
          never completed — open one and use <b>Sync from Razorpay</b> if a
          delegate says they paid.
        </span>
      </div>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center gap-2">
            ...
            <CardTitle>
              Incomplete payments ({pendingPayments.length})
            </CardTitle>
          </div>
        </CardHeader>
        <CardContent className="p-0">
              ...
              {loading ? (
                ...
                <TableRow>
                  <TableCell
                    ...
                    className="text-center py-8 text-sm text-[var(--muted-text)]"
                  >
                    No incomplete payments — every registration is settled.
                  </TableCell>
                </TableRow>
              ) : (
                pendingPayments.map((r) => (
                  <TableRow key={r._id}>
                    ...
                    <TableCell>
                      <Link
                        href={`/editorial/registrations/${r._id}`}
                        className="text-[var(--primary-800)] hover:underline font-semibold"
                      >
                        {r.fullName}
                      </Link>
