...
export async function GET(request: NextRequest) {
  ...
  if (q) {
    ...
    filter.$or = ["title", "submissionCode", "email", "presentingAuthor"].map(
  ...
  const data = rows.map((a) => [
    ...
    a.title,
    a.presentingAuthor,
    a.coAuthors.map((c) => `${c.name} (${c.institution})`).join("; "),
    a.institution,
    a.email,
    a.phone,
    a.aptiMemberId,
    ...
    a.presentationType,
    a.abstractCode,
    a.linkedRegistration ? "Yes" : "No",
    a.createdAt,
  ]);
  const format = url.searchParams.get("format") === "csv" ? "csv" : "xlsx";
  const filename = `APTICON-2026-Abstracts-${new Date().toISOString().slice(0, 10)}.${format}`;
  return format === "csv"
    ? csvResponse(filename, headers, data)
    : excelResponse(filename, "Abstracts", headers, data);
}
