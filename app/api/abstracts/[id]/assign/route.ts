...
export async function PATCH(request: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    const reviewers = await User.find({ _id: { $in: requested }, role: "reviewer", isActive: true });
    if (reviewers.length !== requested.length) {
      return NextResponse.json({ error: "One or more reviewers are invalid" }, { status: 400 });
    }
    ...
    const newlyAdded = reviewers.filter((r) => !before.includes(r._id.toString()));
    await Promise.all(
      newlyAdded.map(async (r) => {
        const assignmentCount = await Abstract.countDocuments({ assignedReviewers: r._id, status: { $ne: "accepted" } });
        const { subject, html } = reviewerAssignmentEmail(r.name, assignmentCount);
