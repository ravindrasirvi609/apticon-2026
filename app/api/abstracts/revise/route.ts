...
import {
  sendMail,
  abstractResubmittedEmail,
  abstractResubmissionNoticeEmail,
} from "@/lib/email";
...
export async function POST(request: NextRequest) {
  ...
  if (!limit.ok) {
    return NextResponse.json(
      { error: "Too many attempts. Please retry in an hour." },
      { status: 429 },
    );
  }
  ...
  const abs = await Abstract.findOne({
    submissionCode: data.code.toUpperCase(),
    email: data.email,
  });
  if (!abs) {
    return NextResponse.json(
  ...
  if (abs.status !== "revision_requested") {
    return NextResponse.json(
      { error: "This abstract is not open for revision." },
      { status: 409 },
    );
  }
  ...
  if (!abs.graphicalAbstractKey && !data.graphicalAbstractKey) {
    return NextResponse.json(
      { error: "A graphical abstract is required for resubmission." },
      { status: 400 },
    );
  }
  ...
  const authorEmail = abstractResubmittedEmail(
  ...
  const noticeEmail = abstractResubmissionNoticeEmail(
    abs.submissionCode,
    abs.title,
    abs.presentingAuthor,
  );
