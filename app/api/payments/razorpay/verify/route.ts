...
export async function POST(request: NextRequest) {
  ...
  if (
    !registration ||
    registration.paymentMode !== "razorpay" ||
    registration.razorpayOrderId !== data.razorpay_order_id
  ) {
    return NextResponse.json(
  ...
  try {
    ...
    if (
      ...
      payment.amount < registration.feeAmount * 100
    ) {
      ...
      return NextResponse.json(
        { error: "Payment details do not match this registration" },
        { status: 400 },
      );
    }
    if (payment.status === "captured") {
      await recordCapturedRazorpayPayment(payment, request);
      const confirmed = await Registration.findById(registration._id)
        .select("registrationCode")
        .lean();
      return NextResponse.json({
        ok: true,
        captured: true,
        registrationCode: confirmed?.registrationCode ?? null,
      });
    }
