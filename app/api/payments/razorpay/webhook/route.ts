...
import {
  recordCapturedRazorpayPayment,
  recordCapturedGroupRazorpayPayment,
    ...
    if (webhook.event === "payment.captured") {
      ...
      if (registration) {
        ...
        if (
          ...
          console.error(
            "[razorpay] webhook payment does not match a registration:",
            {
          ...
          return NextResponse.json(
            { error: "Payment does not match a registration" },
            { status: 400 },
          );
        }
        ...
        if (
          ...
          console.error(
            "[razorpay] webhook payment does not match a registration or group:",
            {
              orderId: payment.order_id,
              paymentId: payment.id,
              receivedAmount: payment.amount,
