...
export async function POST(request: NextRequest) {
  try {
    ...
    const isAssigned = abs.assignedReviewers.some(
      (r) => r.toString() === reviewer.uid,
    );
    if (!isAssigned)
      return NextResponse.json(
        { error: "You are not assigned to this abstract" },
        { status: 403 },
      );

    if (abs.finalDecision === "accepted" || abs.finalDecision === "rejected") {
      return NextResponse.json(
        { error: "This abstract has already been finalized." },
        { status: 409 },
      );
    }
    ...
    const doc =
      ...
      new Review({
        abstract: abs._id,
        reviewer: new mongoose.Types.ObjectId(reviewer.uid),
      });
    ...
    if (parsed.data.verdict === "accept") {
      ...
      await Promise.all(
        recipients.map((u) => sendMail({ to: u.email, subject, html })),
      );
    }
