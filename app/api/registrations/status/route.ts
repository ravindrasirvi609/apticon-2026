...
export async function POST(request: NextRequest) {
  ...
  const reg = await Registration.findOne({
    ...
    .select(
      "registrationCode fullName email category feeAmount feeTier status createdAt approvedAt rejectedAt reviewNote linkedAbstract willSubmitAbstract photoUrl paymentStatus",
    )
    .lean();

  if (!reg) {
    return NextResponse.json(
      { error: "No registration found with that code and email." },
      { status: 404 },
    );
  }
