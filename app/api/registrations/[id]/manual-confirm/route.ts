...
export async function POST(
  ...
  try {
    ...
    await connectDB();
    const source = await Registration.findById(id)
      .select(
        "status category groupRegistration registrationCode paymentError razorpayPaymentId feeAmount",
      )
      .lean();
    if (!source)
    ...
    if (source.groupRegistration)
      return NextResponse.json(
        { error: "Confirm group delegates from the group registration." },
        { status: 400 },
      );
      ...
      if (justApproved) {
        await logAudit({
          ...
          details: {
            ...
            priorPaymentError: source.paymentError ?? null,
          },
          request,
        });
      }
    ...
    if (!reg || reg.status !== "approved")
      return NextResponse.json(
        { error: "Could not confirm this registration." },
        { status: 409 },
      );
