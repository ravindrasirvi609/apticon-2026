...
export async function POST(request: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (reg.paymentMode !== "razorpay" || !reg.razorpayOrderId) {
      return NextResponse.json({ error: "This registration has no Razorpay order to sync." }, { status: 400 });
    }

    const result = await syncRazorpayRegistration(reg, request, { uid: s.uid, role: s.role });
    const fresh = await Registration.findById(id).select("status paymentStatus paymentError").lean();
