...
export default function UsersClient() {
  ...
  const [form, setForm] = useState({ email: "", name: "", role: "reviewer" as UserRole, expertise: "" });
        ...
        actions={
          <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
              <Button><UserPlus className="w-4 h-4" />New user</Button>
            </DialogTrigger>
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Create user</DialogTitle>
                <DialogDescription>A temporary password will be emailed to the address you provide.</DialogDescription>
              </DialogHeader>
              <div className="space-y-4">
                ...
                {form.role === "reviewer" && (
                  <div>
                    <Label htmlFor="exp">Expertise <span className="text-xs font-normal text-[var(--muted-text)]">(comma-separated)</span></Label>
                    <Input id="exp" className="mt-2" placeholder="Clinical Pharmacy, Pharmacology" value={form.expertise} onChange={(e) => setForm({ ...form, expertise: e.target.value })} />
                  </div>
                )}
              </div>
              <DialogFooter>
                <Button variant="outline" onClick={() => setOpen(false)}>Cancel</Button>
                <Button onClick={createUser} disabled={saving || !form.email || !form.name}>
      ...
      <Card>
        <CardContent className="p-0">
          <Table>
            ...
            <TableBody>
              {loading ? (
                <TableRow><TableCell colSpan={7} className="text-center py-8 text-sm text-[var(--muted-text)]">Loading…</TableCell></TableRow>
              ) : users.length === 0 ? (
                <TableRow><TableCell colSpan={7} className="text-center py-8 text-sm text-[var(--muted-text)]">No users yet.</TableCell></TableRow>
              ) : (
                users.map((u) => (
                  <TableRow key={u.id}>
                    ...
                    <TableCell className="text-xs">
                      {u.expertise?.slice(0, 3).map((e) => <Badge key={e} variant="outline" className="text-[10px] mr-1">{e}</Badge>)}
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell className="text-xs text-[var(--muted-text)]">
                      {u.lastLoginAt ? format(new Date(u.lastLoginAt), "d MMM, HH:mm") : "—"}
                    </TableCell>
                    <TableCell className="text-right">
                      <AlertDialog>
                        ...
                        <AlertDialogContent>
                          <AlertDialogHeader>
                            <AlertDialogTitle>{u.isActive ? "Deactivate" : "Reactivate"} {u.name}?</AlertDialogTitle>
