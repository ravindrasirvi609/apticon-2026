...
export default function CheckinClient() {
  ...
  const activeTabDef = REPORT_TABS.find((t) => t.value === activeTab) ?? REPORT_TABS[0];
  ...
  return (
    <div className="p-4 md:p-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
        <StatCard label="Total Registered" value={stats?.totalRegistered ?? "—"} icon={Users} />
        <StatCard label="Checked In" value={stats?.checkedIn ?? 0} icon={UserCheck} accent="emerald" />
        <StatCard label="ID Cards Issued" value={stats?.idCardIssued ?? 0} icon={BadgeCheck} />
        <StatCard label="Kit Distributed" value={stats?.kitDistributed ?? 0} icon={Package} />
        <StatCard label="Certificates Issued" value={stats?.certificatesDistributed ?? 0} icon={Award} />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        <MealByDayCard title="Breakfast by Day" counts={stats?.breakfast} total={stats?.checkedIn} />
        <MealByDayCard title="Lunch by Day" counts={stats?.lunch} total={stats?.checkedIn} />
        <MealByDayCard title="Dinner by Day" counts={stats?.dinner} total={stats?.checkedIn} />
      </div>
              ...
              {activeTabDef.dayScoped && (
                <div className="flex items-center gap-2">
                  ...
                  <Input
                    ...
                    className="w-20"
                    value={day}
                    onChange={(e) => { setDay(Math.max(1, Number(e.target.value) || 1)); setPage(1); }}
                  />
                </div>
              )}
            </div>
              ...
              {report && report.total > report.limit && (
                <div className="flex items-center justify-between mt-4">
                  <span className="text-xs text-[var(--muted-text)]">
                    {(page - 1) * report.limit + 1}–{Math.min(page * report.limit, report.total)} of {report.total}
                  </span>
                  <div className="flex gap-2">
                    <Button variant="outline" size="sm" disabled={page <= 1} onClick={() => setPage((p) => p - 1)}>Prev</Button>
                    <Button variant="outline" size="sm" disabled={page * report.limit >= report.total} onClick={() => setPage((p) => p + 1)}>Next</Button>
                  </div>
                </div>
              )}
            </TabsContent>
...
function MealByDayCard({ title, counts, total }: { title: string; counts?: Record<string, number>; total?: number }) {
    ...
    <Card>
      ...
      <CardContent>
        <div className="space-y-3">
          ...
          {days.length === 0 && <p className="text-sm text-[var(--muted-text)]">No data yet.</p>}
        </div>
      </CardContent>
    </Card>
  );
