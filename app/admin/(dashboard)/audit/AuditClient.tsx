        ...
        <CardContent className="pt-6">
          <div className="flex flex-col md:flex-row gap-3">
            <div className="flex flex-wrap gap-2">
              <div className="text-xs font-semibold uppercase tracking-wider text-[var(--muted-text)] self-center">
                Resource:
              </div>
              {RESOURCE_TYPES.map((s) => (
                <Button
                  key={s || "all-res"}
                  variant={resourceType === s ? "default" : "outline"}
                  size="sm"
                  onClick={() => setResourceType(s)}
                >
                  {s || "All"}
                </Button>
              ))}
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {ACTOR_ROLES.map((s) => (
                <Button
                  key={s || "all-act"}
                  variant={actorRole === s ? "default" : "outline"}
                  size="sm"
                  onClick={() => setActorRole(s)}
                >
                  {s ? s.replace("_", " ") : "All"}
                </Button>
              ))}
                ...
                items.map((row) => {
                  ...
                  return (
                    <TableRow key={row._id}>
                      ...
                      <TableCell className="text-xs font-mono text-[var(--muted-text)]">
                        {row.ip}
                      </TableCell>
                      <TableCell className="text-xs max-w-md">
                        {row.details ? (
                          <details>
                            <summary className="cursor-pointer text-[var(--primary-800)] hover:underline">
                              view
                            </summary>
