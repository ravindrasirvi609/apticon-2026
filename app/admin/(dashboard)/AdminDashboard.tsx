  ...
  return (
    <div className="p-4 md:p-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
        ...
        <StatCard
          label="Approved · Paid"
          value={stats?.registrationsByStatus.approved ?? 0}
          icon={CheckCircle2}
          accent="emerald"
        />
        <StatCard
          ...
          value={
            stats
              ? "₹" + (stats.totals.revenue ?? 0).toLocaleString("en-IN")
              : "—"
          }
          icon={IndianRupee}
        />
        <StatCard
          label="APTI Membership + Reg."
          value={stats?.totals.aptiMembershipRegistrations ?? 0}
          icon={BadgeCheck}
          accent="emerald"
        />
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <StatCard
          label="Abstracts"
          value={stats?.totals.abstracts ?? "—"}
          icon={FileText}
        />
        <StatCard
          label="Under Review"
          value={stats?.abstractsByStatus.under_review ?? 0}
          icon={Clock}
          accent="amber"
        />
        <StatCard
          label="Accepted"
          value={stats?.abstractsByStatus.accepted ?? 0}
          icon={CheckCircle2}
          accent="emerald"
        />
        <StatCard
              ...
              <TableBody>
                ...
                {stats && stats.recentRegistrations.length === 0 && (
                  <TableRow>
                    <TableCell
                      colSpan={4}
                      className="text-center text-sm text-[var(--muted-text)] py-6"
                    >
      ...
      <Card>
        <CardHeader>
          <CardTitle>Abstracts by Theme</CardTitle>
        </CardHeader>
        <CardContent>
              ...
              <div key={t.theme}>
                ...
                <div className="h-1.5 bg-[var(--surface-100)] rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${Math.min(100, (t.count / Math.max(1, stats?.totals.abstracts ?? 1)) * 100)}%`,
                    }}
                  />
                </div>
              </div>
