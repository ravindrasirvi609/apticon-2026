  ...
  async function submitRevision() {
    ...
    try {
      ...
      const res = await fetch("/api/abstracts/revise", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ code, email, abstract: abstractText, fileKey, fileName, graphicalAbstractKey, graphicalAbstractName }),
      });
      ...
      {record && !done && record.status === "revision_requested" && (
        <Card className="max-w-2xl mx-auto mt-8">
          <CardHeader>
            ...
            <CardDescription className="font-mono">{record.submissionCode}</CardDescription>
          </CardHeader>
          <CardContent className="space-y-5">
            ...
            <div>
              ...
              />
              {abstractText.trim().length < 100 && <p className={errCls}>Abstract must be at least 100 characters.</p>}
            </div>
            ...
            <Button onClick={submitRevision} size="lg" className="w-full" disabled={submitting}>
