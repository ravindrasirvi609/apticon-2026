...
export default function AbstractsClient() {
  ...
  } = useFieldArray({ control, name: "coAuthors" });
  ...
  const onSubmit = async (data: AbstractForm) => {
    ...
    const res = await fetch("/api/abstracts", {
      ...
      body: JSON.stringify({
        ...
        fileKey,
        fileName,
        graphicalAbstractKey: uploadedGraphicalAbstractKey,
        graphicalAbstractName: uploadedGraphicalAbstractName,
      }),
    });
      ...
      <section className="pt-8 pb-12 px-4 text-center">
        <ScrollReveal>
          ...
          <p className="mt-4 max-w-2xl mx-auto text-base md:text-lg text-[var(--muted-text)]">
            Share your work with 1500+ pharmacy educators and researchers at
            APTICON 2026, Raipur.
          </p>
          <div className="mt-7 flex flex-wrap items-center justify-center gap-3 text-left">
            {[
              ...
              <div
                key={label}
                className="inline-flex items-center gap-2 rounded-full border border-[var(--accent-500)]/30 bg-white/80 px-4 py-2 text-sm font-medium text-[var(--dark-text)] shadow-sm"
              >
                <Icon
                  className="h-4 w-4 text-[var(--primary-800)]"
                  aria-hidden
                />
                {label}
              </div>
      ...
      <section className="py-12 px-4 max-w-4xl mx-auto">
        ...
        >
          {IMPORTANT_DATES.map((d) => (
            ...
            >
              <span className="text-sm md:text-base text-[var(--dark-text)]">
                {d.event}
              </span>
              <span className="text-sm md:text-base font-semibold text-[var(--primary-800)]">
                {d.date}
              </span>
            </motion.li>
          ))}
        ...
        <ScrollReveal>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {ABSTRACT_THEMES.map((t) => (
              <div
                key={t}
                className="px-4 py-3 rounded-lg bg-[var(--surface-100)] border border-[var(--accent-500)]/25 text-sm text-[var(--dark-text)]"
              >
                {t}
              </div>
            ))}
          ...
          <CardContent className="pt-8">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
              <div>
                ...
                />
                {errors.title && (
                  <p className={errCls}>Title is required (5–300 chars).</p>
                )}
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                  ...
                  <Input
                    id="presentingAuthor"
                    className="mt-2"
                    {...register("presentingAuthor", { required: true })}
                  />
              ...
              <div>
                ...
                <div className="mt-2 space-y-3">
                  {coAuthorFields.map((field, index) => (
                    <div
                      key={field.id}
                      className="rounded-lg border border-[var(--accent-500)]/30 bg-white p-3"
                    >
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div>
                          <Label
                            htmlFor={`coAuthors.${index}.name`}
                            className="text-xs font-normal text-[var(--muted-text)]"
                          >
                            Name *
                          </Label>
                          <Input
                        ...
                        <div>
                          <Label
                            htmlFor={`coAuthors.${index}.institution`}
                            className="text-xs font-normal text-[var(--muted-text)]"
                          >
                            Institution *
                          </Label>
                          <Input
                            ...
                            className="mt-1"
                            {...register(`coAuthors.${index}.institution`, {
                              required: true,
                              minLength: 2,
                            })}
                          />
                          {errors.coAuthors?.[index]?.institution && (
                            <p className={errCls}>Required.</p>
                          )}
                        </div>
                      </div>
                  ...
                  {coAuthorFields.length === 0 && (
                    <p className="text-xs text-[var(--muted-text)]">
                      No co-authors added. Click "Add Co-Author" if this work
                      has other authors besides the presenting author.
                    </p>
                  )}
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div>
                  ...
                  {errors.phone && (
                    <p className={errCls}>
                      {errors.phone.message || "Enter a valid 10-digit mobile."}
                    </p>
                  )}
                </div>
              </div>
              ...
              <div>
                <Label>
                  ...
                  <span className="text-xs font-normal text-[var(--muted-text)]">
                    (optional — editorial confirms this on acceptance)
                  </span>
                </Label>
                <div className="mt-2 flex gap-3">
                  <label className="flex-1 flex items-center gap-2 px-4 py-2 rounded-lg border border-[var(--accent-500)]/30 bg-white cursor-pointer hover:border-[var(--primary-800)]/40">
                    <input
                      type="radio"
                      value="oral"
                      {...register("preferredPresentationType")}
                    />
              ...
              <div>
                ...
                <Input
                  id="keywords"
                  className="mt-2"
                  placeholder="nanoparticles, drug delivery, controlled release"
                  {...register("keywords", { required: true })}
                />
                {errors.keywords && (
                  <p className={errCls}>Enter at least one keyword.</p>
                )}
              </div>
              ...
              <div>
                <div className="flex items-center justify-between">
                  ...
                  <span
                    className={`text-xs ${wordCount > MAX_ABSTRACT_WORDS ? "text-red-600" : "text-[var(--muted-text)]"}`}
                  >
                ...
                {errors.abstract && (
                  <p className={errCls}>
                    {errors.abstract.message ??
                      "Abstract is required (100–3800 characters)."}
                  </p>
                )}
                <p className="mt-2 text-xs text-[var(--muted-text)]">
                  Note: Your abstract should clearly cover the Objectives,
                  Methods, Results, and Conclusion of your work.
                </p>
              </div>
              ...
              <div>
                <Label htmlFor="file">
                  Abstract file * (DOC or DOCX, max 10 MB)
                </Label>
                <div className="mt-2 flex items-center gap-3">
                  <label className="flex-1 flex items-center gap-2 px-4 py-2.5 rounded-lg border border-dashed border-[var(--accent-500)]/40 bg-white cursor-pointer hover:border-[var(--primary-800)]/40 transition-colors">
                    <Upload className="w-4 h-4 text-[var(--muted-text)]" />
