    ...
    <section className="py-20 md:py-24 bg-[var(--cream-100)]">
      <div className="container-site">
        <ScrollReveal className="text-center mb-12">
          <p className="text-xs font-bold tracking-widest uppercase text-[var(--gold-500)] mb-3">Conference Program</p>
          <h2 className="font-display font-black text-3xl sm:text-4xl md:text-5xl text-[var(--dark-text)]">
            Two Days of <span className="text-gradient-crimson">Excellence</span>
          </h2>
        ...
        >
          {DAY_HIGHLIGHTS.map((day) => (
            <motion.div
              key={day.day}
              variants={fadeUp}
              className="rounded-2xl bg-white border border-[var(--gold-500)]/15 shadow-sm overflow-hidden hover:shadow-md transition-shadow duration-300"
            >
              {/* Day header */}
              <div className={`bg-gradient-to-r ${day.color} p-5`}>
                <p className="text-[var(--gold-400)] text-xs font-bold tracking-widest uppercase mb-1">{day.date}</p>
                <h3 className="font-display font-black text-2xl text-white">{day.day}</h3>
              </div>

              {/* Sessions list */}
              <div className="divide-y divide-[var(--gold-500)]/10">
                {day.sessions.map((session, i) => {
                  const colorCls = SESSION_COLORS[session.type] ?? "bg-gray-100 text-gray-700";
                  return (
                    <div key={i} className="flex gap-4 items-start p-4 hover:bg-[var(--cream-50)] transition-colors duration-150">
              ...
              {/* Footer CTA */}
              <div className="p-4 border-t border-[var(--gold-500)]/10 bg-[var(--cream-50)]">
                <Link
                  href="/schedule"
                  className="flex items-center gap-2 text-sm font-semibold text-[var(--crimson-800)] hover:gap-3 transition-all duration-200"
                >
        ...
        <ScrollReveal className="mt-10 text-center">
          <Link
            href="/schedule"
            className="inline-flex items-center gap-2 px-8 py-3.5 rounded-xl bg-[var(--crimson-800)] text-white font-bold text-sm hover:bg-[var(--crimson-700)] transition-colors shadow-md focus:outline-none focus:ring-2 focus:ring-[var(--gold-400)] focus:ring-offset-2"
          >
