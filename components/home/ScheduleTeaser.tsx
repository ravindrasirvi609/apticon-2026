...
const DAY_HIGHLIGHTS = [
  {
    ...
    sessions: SCHEDULE_DAY1.filter(
      (s) => !["break", "logistics"].includes(s.type),
    ).slice(0, 4),
  },
  {
    day: "Day 2",
      ...
      <div className="container-site">
        <ScrollReveal className="text-center mb-12">
          ...
          <p className="mt-4 text-[var(--muted-text)] max-w-md mx-auto text-sm sm:text-base">
            Keynotes, scientific sessions, workshops, cultural evenings — a
            packed agenda awaits.
          </p>
        </ScrollReveal>
          ...
          {DAY_HIGHLIGHTS.map((day) => (
            ...
            >
              ...
              <div className={`bg-gradient-to-r ${day.color} p-5`}>
                <p className="text-[var(--accent-400)] text-xs font-bold tracking-widest uppercase mb-1">
                  {day.date}
                </p>
              ...
              <div className="divide-y divide-[var(--accent-500)]/10">
                {day.sessions.map((session, i) => {
                  const colorCls =
                    SESSION_COLORS[session.type] ?? "bg-gray-100 text-gray-700";
                  return (
                    <div
                      key={i}
                      className="flex gap-4 items-start p-4 hover:bg-[var(--surface-50)] transition-colors duration-150"
                    >
                      <div className="flex flex-col items-center gap-1 flex-shrink-0 min-w-[56px]">
                        <Clock size={12} className="text-[var(--muted-text)]" />
                        <span className="text-[10px] font-semibold text-[var(--muted-text)] text-center leading-tight">
