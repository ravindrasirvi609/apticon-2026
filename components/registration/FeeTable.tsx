const FEES = [
  ...
  {
    category: "PG Student / Research Scholar",
    early: "₹2,500",
    regular: "₹3,000",
    spot: "₹3,500",
  },
...
const DATES = [
  ...
  },
  {
    ...
    date: "16 Sep – 10 Oct 2026",
    color: "text-[var(--secondary-800)]",
  },
  {
    label: "On-Spot Registration",
    date: "24–25 October 2026",
    color: "text-[var(--primary-800)]",
  },
];
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {DATES.map((d) => (
          <div
            key={d.label}
            className="rounded-xl bg-white border border-[var(--accent-500)]/20 p-4"
          >
            <p className="text-xs font-bold tracking-widest uppercase text-[var(--muted-text)] mb-1">
              {d.label}
            </p>
            <p className={`font-display font-bold text-lg ${d.color}`}>
              {d.date}
            </p>
          </div>
        ))}
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-[var(--primary-800)] text-white">
              <th className="text-left px-5 py-4 font-semibold text-xs uppercase tracking-wide">
                Category
              </th>
              <th className="text-center px-4 py-4 font-semibold text-xs uppercase tracking-wide whitespace-nowrap">
                Early Bird
                <br />
                <span className="text-[var(--accent-400)] normal-case text-[10px]">
                  Till 15 Sep
                </span>
              </th>
              <th className="text-center px-4 py-4 font-semibold text-xs uppercase tracking-wide whitespace-nowrap">
      ...
      <div className="space-y-2 text-xs text-[var(--muted-text)]">
        <p>
          * All amounts shown here are in Indian Rupees (INR). GST shall be
          applicable extra, as per prevailing government regulations.
        </p>
        <p>
          <span className="font-semibold text-[var(--dark-text)]">Remark:</span>{" "}
          Accompanying persons will not be entitled for issuance of Registration
          Kit, however they will be provided the access to Food Court.
        </p>
      </div>
    </div>
  );
