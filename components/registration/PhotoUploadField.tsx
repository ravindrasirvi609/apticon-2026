    ...
    <div>
      ...
      <div
        ...
        className={`group relative mt-2 overflow-hidden rounded-2xl border-2 border-dashed p-4 transition-all sm:p-5 ${
          message ? "border-red-400 bg-red-50/50" : isDragging
      ...
      >
        <div className="flex items-center gap-4">
          <div className="relative h-20 w-20 shrink-0 overflow-hidden rounded-xl bg-[var(--surface-100)] ring-1 ring-black/5 sm:h-24 sm:w-24">
            ...
            ) : <div className="flex h-full w-full items-center justify-center"><User className="h-8 w-8 text-[var(--muted-text)]/40" aria-hidden /></div>}
            {file && <span className="absolute bottom-1 right-1 rounded-full bg-white p-0.5 text-emerald-600 shadow"><CheckCircle2 className="h-4 w-4" /></span>}
          </div>
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2 text-sm font-semibold text-[var(--dark-text)]">
              {isDragging ? "Drop your photo here" : file ? "Photo ready to upload" : "Add your profile photo"}
              {!file && <ImagePlus className="h-4 w-4 text-[var(--accent-500)]" aria-hidden />}
            </div>
            <p className="mt-1 text-xs leading-5 text-[var(--muted-text)]">
              {file ? <span className="block truncate font-medium text-[var(--primary-800)]">{file.name}</span> : <>Drag & drop here, or <span className="font-semibold text-[var(--primary-800)]">browse files</span></>}
              <span className="block">{UPLOAD_RULES.photo.label} · max {MAX_MB} MB</span>
            </p>
          </div>
          <UploadCloud className={`hidden h-7 w-7 shrink-0 transition-colors sm:block ${isDragging ? "text-[var(--primary-800)]" : "text-[var(--accent-500)]/60"}`} aria-hidden />
        </div>
        {file && <Button type="button" variant="ghost" size="sm" disabled={disabled} onClick={(event) => { event.stopPropagation(); clear(); }} className="absolute right-2 top-2"><X className="h-4 w-4" />Remove</Button>}
        <input ref={inputRef} id="photo" type="file" accept={ACCEPT} className="hidden" disabled={disabled} onChange={(e) => pick(e.target.files?.[0] ?? null)} />
      </div>
