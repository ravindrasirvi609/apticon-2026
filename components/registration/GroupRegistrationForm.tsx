...
const EMPTY_DELEGATE: DelegateRow = {
  name: "",
  designation: "",
  email: "",
  phone: "",
...
export default function GroupRegistrationForm() {
  ...
  const [photos, setPhotos] = useState<(File | null)[]>(
    Array.from({ length: GROUP_MIN_DELEGATES }, () => null),
  );
  const [coordinatorPhoto, setCoordinatorPhoto] = useState<File | null>(null);
  const [coordinatorPhotoError, setCoordinatorPhotoError] = useState<
    string | null
  >(null);
  ...
  const {
    ...
    defaultValues: {
      delegates: Array.from({ length: GROUP_MIN_DELEGATES }, () => ({
        ...EMPTY_DELEGATE,
      })),
    },
  });
  ...
  const requiresMembershipId =
    ...
    APTI_MEMBER_CATEGORIES.includes(
      category as (typeof APTI_MEMBER_CATEGORIES)[number],
    );
  ...
  const groupFee = chosenCategory
    ? currentGroupFeeAmount(chosenCategory, delegateCount)
    : null;
  const feeBreakdown = groupFee
    ? calculateFeeWithGst(groupFee.baseAmount)
    : null;

  function addDelegate() {
    if (fields.length >= GROUP_MAX_DELEGATES) {
      toast.error(
        `A single group registration supports up to ${GROUP_MAX_DELEGATES} delegates.`,
      );
      return;
    }
  ...
  async function uploadPhoto(file: File): Promise<string | null> {
    ...
    if (!res.ok) {
      toast.error(
        body?.error ?? "A delegate photo upload failed. Please try again.",
      );
      return null;
    }
  ...
  const onSubmit = async (data: FormData) => {
    ...
    const res = await fetch("/api/payments/razorpay/group-order", {
      ...
      body: JSON.stringify({
        ...
        coordinatorAptiMemberId: requiresMembershipId
        ...
        handler: async (response) => {
          ...
          try {
            const verify = await fetch("/api/payments/razorpay/group-verify", {
              ...
              headers: { "content-type": "application/json" },
              body: JSON.stringify({
                groupRegistrationId: body.groupRegistrationId,
                ...response,
              }),
            });
            ...
            toast.success(
              "Payment confirmed. Your group registration is confirmed. Confirmation emails are on the way.",
            );
            router.push(
              `/registration/group/success/${result.groupCode}?payment=${result.captured ? "confirmed" : "processing"}`,
            );
          } catch (error) {
            toast.error(
            ...
            router.push(
              `/registration/group/success/${body.groupCode}?payment=processing`,
            );
          } finally {
            setPaying(false);
          }
    ...
    <form onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            {errors.coordinatorName && (
              <p className={errCls}>{errors.coordinatorName.message}</p>
            )}
          </div>
          <div>
            ...
            {errors.coordinatorPhone && (
              <p className={errCls}>{errors.coordinatorPhone.message}</p>
            )}
          </div>
          <div className="sm:col-span-2">
            ...
            {errors.coordinatorEmail && (
              <p className={errCls}>{errors.coordinatorEmail.message}</p>
            )}
            <p className="mt-1 text-xs text-[var(--muted-text)]">
              Payment receipt and group confirmation are sent here.
            </p>
          </div>
          <div className="sm:col-span-2">
            <Label htmlFor="coordinatorAffiliation">
              Coordinator Affiliation *
            </Label>
            <Input
              id="coordinatorAffiliation"
              className="mt-2"
              placeholder="Department / organisation / institution"
              {...register("coordinatorAffiliation", {
                required: "Affiliation is required",
                minLength: 2,
              })}
            />
            {errors.coordinatorAffiliation && (
              <p className={errCls}>Affiliation is required.</p>
            )}
          </div>
          <div className="sm:col-span-2">
            <PhotoUploadField
              file={coordinatorPhoto}
              onChange={(f) => {
                setCoordinatorPhoto(f);
                if (f) setCoordinatorPhotoError(null);
              }}
              error={coordinatorPhotoError ?? undefined}
              disabled={busy}
            />
          </div>
          {requiresMembershipId && (
            <div className="sm:col-span-2">
              <AptiMembershipIdField
                registerProps={register("coordinatorAptiMemberId", {
                  validate: (v) =>
                    !requiresMembershipId ||
                    (v?.trim().length ?? 0) >= 3 ||
                    "APTI Membership ID is required",
                })}
                error={errors.coordinatorAptiMemberId?.message}
                helperText="This category requires an APTI Membership ID for the coordinator. We will verify it against the APTI registry."
              />
            </div>
          )}
          <div className="sm:col-span-2">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <Label htmlFor="category">
              Category (applies to whole group) *
            </Label>
            <select
              id="category"
              className="mt-2 flex h-10 w-full rounded-lg border border-[var(--accent-500)]/30 bg-white px-3 py-2 text-sm text-[var(--dark-text)] focus:outline-none focus:ring-2 focus:ring-[var(--accent-400)]"
            ...
            >
              ...
              {REGISTRATION_CATEGORIES.filter(
                (c) =>
                  ...
                  c !== "APTI Life Membership + APTICON Registration",
              ).map((c) => (
                ...
                <div className="space-y-1 text-sm">
                  <div className="flex items-center justify-between gap-3">
                    <span className="text-[var(--muted-text)]">
                      ...
                      {formatRupees(groupFee.perHead)}
                    </span>
                    <span>
                      {formatRupees(groupFee.perHead * delegateCount)}
                    </span>
                  </div>
                  {groupFee.complimentaryCount > 0 && (
                    <div className="flex items-center justify-between gap-3 text-emerald-700">
                      ...
                      <span>
                        −
                        {formatRupees(
                          groupFee.perHead * groupFee.complimentaryCount,
                        )}
                      </span>
                    </div>
                  )}
                  ...
                  <span className="block text-[10px] text-[var(--muted-text)] uppercase tracking-wider">
                    {groupFee.tier.replace("_", " ")} · {groupFee.paidCount}{" "}
                    payable
                  </span>
                </div>
              ) : (
      ...
      <div>
        ...
        <div className="space-y-5">
          {fields.map((field, index) => (
            <div
              key={field.id}
              className="rounded-xl border border-[var(--accent-500)]/20 p-4 bg-[var(--surface-50)]/50"
            >
              <div className="flex items-center justify-between mb-3">
                <span className="text-xs font-bold uppercase tracking-wider text-[var(--muted-text)]">
                  Delegate #{index + 1}
                  {index >=
                    delegateCount - (groupFee?.complimentaryCount ?? 0) &&
                  (groupFee?.complimentaryCount ?? 0) > 0
                    ? " · Complimentary"
                    : ""}
                </span>
                <Button
                  ...
                  disabled={busy || fields.length <= GROUP_MIN_DELEGATES}
                >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <Input
                    id={`delegates.${index}.name`}
                    className="mt-1.5"
                    {...register(`delegates.${index}.name`, {
                      required: true,
                      minLength: 2,
                    })}
                  />
                ...
                <div className="sm:col-span-2">
                  <Label htmlFor={`delegates.${index}.affiliation`}>
                    Affiliation *
                  </Label>
                  <Input
                    id={`delegates.${index}.affiliation`}
                    className="mt-1.5"
                    placeholder="Department / organisation / institution"
                    {...register(`delegates.${index}.affiliation`, {
                      required: true,
                      minLength: 2,
                    })}
                  />
                  {errors.delegates?.[index]?.affiliation && (
                    <p className={errCls}>Affiliation is required.</p>
                  )}
                </div>
                {requiresMembershipId && (
                  <div className="sm:col-span-2">
                    <AptiMembershipIdField
                      ...
                      error={errors.delegates?.[index]?.aptiMemberId?.message}
                      helperText="This category requires an APTI Membership ID for every delegate. We will verify it against the APTI registry."
                    />
                  </div>
                )}
                <div>
                  <Label htmlFor={`delegates.${index}.designation`}>
                    Designation *
                  </Label>
                  <Input
                    id={`delegates.${index}.designation`}
                    className="mt-1.5"
                    {...register(`delegates.${index}.designation`, {
                      required: true,
                      minLength: 2,
                    })}
                  />
                  {errors.delegates?.[index]?.designation && (
                    <p className={errCls}>Required.</p>
                  )}
                </div>
                <div>
                  ...
                  <Input
                    id={`delegates.${index}.email`}
                    type="email"
                    className="mt-1.5"
                    {...register(`delegates.${index}.email`, {
                      required: true,
                      pattern: /^[^\s@]+@[^\s@]+\.[^\s@]+$/,
                    })}
                  />
                  {errors.delegates?.[index]?.email && (
                    <p className={errCls}>Valid email required.</p>
                  )}
                </div>
                <div>
                  <Label htmlFor={`delegates.${index}.phone`}>Mobile *</Label>
                  <Input
                    id={`delegates.${index}.phone`}
                    type="tel"
                    inputMode="numeric"
                    maxLength={10}
                    className="mt-1.5"
                    {...register(`delegates.${index}.phone`, {
                      required: true,
                      pattern: /^[6-9]\d{9}$/,
                    })}
                    onInput={(e) => {
                  ...
                  />
                  {errors.delegates?.[index]?.phone && (
                    <p className={errCls}>Valid 10-digit mobile required.</p>
                  )}
                </div>
                <div className="sm:col-span-2">
                  <PhotoUploadField
          ...
          <div className="flex items-start gap-2">
            ...
            <div>
              ...
              <p className="text-[var(--muted-text)]">
                After you submit, Razorpay will open its secure checkout for the
                discounted group total. Your group confirmed automatically after
                payment — each delegate receives their own registration code and
                QR badge by email once confirmed.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="pt-2">
        ...
        <p className="mt-3 text-center text-xs text-[var(--muted-text)]">
          Your group registration is confirmed automatically after successful
          payment.
        </p>
      </div>
    </form>
