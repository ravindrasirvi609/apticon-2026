 ...
 */
export default function RegistrationStatusBadge({ status, paymentStatus }: { status: string; paymentStatus?: string }) {
  const { label, variant } = resolve(status, paymentStatus);
  return <Badge variant={variant}>{label}</Badge>;
}

function resolve(status: string, paymentStatus?: string): { label: string; variant: Variant } {
  if (status === "approved") return { label: "Approved · Paid", variant: "success" };

  switch (paymentStatus) {
    case "failed":     return { label: "Payment Failed",   variant: "danger" };
    case "refunded":   return { label: "Refunded",         variant: "secondary" };
    case "authorized": return { label: "Awaiting Capture", variant: "warning" };
    // Captured but still not approved means a mismatch the sync refused to honour — it needs a look.
    case "captured":   return { label: "Captured · Needs Review", variant: "warning" };
  }

  if (status === "submitted") return { label: "Awaiting Payment", variant: "info" };
