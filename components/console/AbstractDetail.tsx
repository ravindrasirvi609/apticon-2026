...
export default function AbstractDetail({
  ...
  const [data, setData] = useState<{
    ...
    reviews: ReviewDoc[];
    reviewers: ReviewerLite[];
    linkedRegistration: LinkedRegLite | null;
  } | null>(null);
  ...
  async function load() {
    ...
    if (uRes.ok) {
      ...
      setAllReviewers(
        u.users
          .filter(
            (x: { role: string; isActive: boolean }) =>
              x.role === "reviewer" && x.isActive,
          )
          .map(
            (x: {
              id: string;
    ...
    <div className="p-4 md:p-8 max-w-6xl">
          ...
          <Card>
            ...
            <CardContent>
              ...
              {a.graphicalAbstractUrl && (
                <div className="mt-4">
                  <span className="text-xs uppercase tracking-wider text-[var(--muted-text)]">
                    Graphical Abstract
                  </span>
              ...
              {data.reviews.map((r) => {
                const rev =
                  typeof r.reviewer === "object"
                    ? r.reviewer
                    : { name: r.reviewer, email: "" };
                return (
                  <div
                    key={r._id}
                    className="rounded-lg border border-[var(--accent-500)]/20 p-4"
                  >
                    <div className="flex items-start justify-between gap-3 mb-3">
                      <div>
                        <div className="font-semibold text-sm">{rev.name}</div>
                        <div className="text-xs text-[var(--muted-text)]">
                          {format(new Date(r.submittedAt), "d MMM yyyy, HH:mm")}
                        </div>
                      </div>
                      <Badge
                        variant={
            ...
            <CardContent>
              ...
              data.linkedRegistration.registrationCode ? (
                <div className="space-y-2 text-sm">
                  <div>
                    <Link
                      href={`${registrationDetailBase}/${data.linkedRegistration._id}`}
                      className="font-mono text-xs text-[var(--primary-800)] hover:underline"
                    >
                      {data.linkedRegistration.registrationCode}
                    </Link>
                  </div>
                  <div className="font-semibold">
                    {data.linkedRegistration.fullName}
                  </div>
                  <div className="text-xs text-[var(--muted-text)]">
                    {data.linkedRegistration.email}
                  </div>
                  <div>
                    {data.linkedRegistration.status && (
                      <RegistrationStatusBadge
                        status={data.linkedRegistration.status}
                        paymentStatus={data.linkedRegistration.paymentStatus}
                      />
                    )}
                  </div>
                  <div className="text-xs text-[var(--muted-text)]">
                    {data.linkedRegistration.feeAmount
                      ? `₹${data.linkedRegistration.feeAmount.toLocaleString("en-IN")} · ${data.linkedRegistration.feeTier?.replace("_", " ")}`
                      : ""}
                  </div>
                  {!a.linkedRegistration && (
                    <div className="mt-2 text-xs text-amber-700">
                      Auto-matched by email — not yet formally linked. Open the
                ...
                <div className="text-sm text-amber-700">
                  ...
                  <div className="text-[var(--muted-text)] mt-1">
                    No registration found for <b>{a.email}</b>. Presenting
                    authors must be registered delegates.
                  </div>
                </div>
              )}
            </CardContent>
              ...
              <Field
                ...
                value={
                  a.preferredPresentationType
                    ? a.preferredPresentationType[0].toUpperCase() +
                      a.preferredPresentationType.slice(1)
                    : "—"
                }
              />
            </CardContent>
                ...
                <DialogContent>
                  ...
                  <div className="max-h-96 overflow-y-auto space-y-2">
                    {allReviewers.map((r) => (
                      <label
                        key={r._id}
                        className="flex items-center gap-3 p-3 rounded-lg border border-[var(--accent-500)]/20 cursor-pointer hover:bg-[var(--surface-50)]"
                      >
                        <Checkbox
                    ...
                    {allReviewers.length === 0 && (
                      <p className="text-sm text-[var(--muted-text)] p-4 text-center">
                        No active reviewers. Create one in Users.
                      </p>
                    )}
                  </div>
                  <DialogFooter>
          ...
          <Card>
            ...
            <CardContent>
                ...
                <div className="space-y-2">
                  ...
                  {a.abstractCode && (
                    <div className="text-sm">
                      <span className="text-xs uppercase tracking-wider text-[var(--muted-text)]">
                        Abstract Code ({a.presentationType})
                      </span>
                  ...
                  {a.finalDecisionAt && (
                    <div className="text-xs text-[var(--muted-text)]">
                      Recorded{" "}
                      {format(new Date(a.finalDecisionAt), "d MMM yyyy, HH:mm")}
                    </div>
                  )}
                  {a.finalDecisionNote && (
              ...
              <div className="mt-4 grid grid-cols-1 gap-2">
                ...
                <Button
                  variant="outline"
                  onClick={() => setDecisionOpen("revision_requested")}
                >
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              Confirm decision: {decisionOpen?.replace("_", " ")}
            </DialogTitle>
          </DialogHeader>
