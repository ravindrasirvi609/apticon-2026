...
export default function GroupRegistrationDetail({
  id,
  backHref,
  ...
  async function syncPayment() {
    setSyncing(true);
    try {
      const res = await fetch(`/api/group-registrations/${id}/sync-payment`, {
        method: "POST",
      });
      ...
      if (body.outcome === "captured") {
        ...
        toast.info(
          "Already approved — every delegate already has their registration.",
        );
  ...
  const statusInfo = STATUS_LABEL[group.status] ?? {
    label: group.status,
    variant: "secondary" as const,
  };
  const missingDelegates =
    group.status === "approved" &&
    group.createdRegistrations.length < group.delegateCount;
    ...
    <div className="p-4 md:p-8 max-w-6xl">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          <Card>
            ...
            <CardContent className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
              ...
              <Field
                ...
                value={group.coordinatorPhone}
              />
              <Field label="Institution" value={group.institution} />
              <Field
                label="City / State"
                value={`${group.city ?? "—"}${group.state ? ", " + group.state : ""}`}
              />
              <Field label="Category" value={group.category} />
              <Field
                label="Delegates"
                value={`${group.delegateCount} (${group.complimentaryCount} complimentary)`}
              />
              <Field
                label="Fee"
                value={`₹${group.feeAmount.toLocaleString("en-IN")} (${group.feeTier.replace("_", " ")})`}
              />
              <Field
                label="Submitted"
                value={format(new Date(group.createdAt), "d MMM yyyy, HH:mm")}
              />
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader>
              <CardTitle>Delegates ({group.delegates.length})</CardTitle>
            </CardHeader>
          ...
          {group.createdRegistrations.length > 0 && (
            <Card>
              <CardHeader>
                <CardTitle>Created Registrations</CardTitle>
              </CardHeader>
              <CardContent>
                <p className="text-sm text-[var(--muted-text)] mb-2">
                  {group.createdRegistrations.length} individual registration
                  {group.createdRegistrations.length === 1 ? "" : "s"} were
                ...
                {registrationDetailBase && (
                  <div className="flex flex-wrap gap-2">
                    {group.createdRegistrations.map((rid) => (
                      <Link
                        key={rid}
                        href={`${registrationDetailBase}/${rid}`}
          ...
          <Card>
            ...
            <CardContent className="space-y-2 text-sm">
              ...
              <Field
                ...
                value={group.razorpayOrderId ?? "—"}
              />
              <Field
                label="Paid at"
                value={
                  group.paidAt
                    ? format(new Date(group.paidAt), "d MMM yyyy, HH:mm")
                    : "—"
                }
              />
              {group.paymentError && (
              ...
              {missingDelegates && (
                <div className="p-3 rounded bg-amber-50 border border-amber-200 text-sm">
                  <div className="font-semibold text-amber-800">
                    {group.createdRegistrations.length} of {group.delegateCount}{" "}
                    delegates have a registration
                  </div>
                  <div className="mt-1 text-amber-900">
                    The rest are missing their confirmation email, code and QR —
                    click Sync to retry them.
                  </div>
                </div>
              )}
              <Button
              ...
              <p className="text-xs text-[var(--muted-text)]">
                Use this if the coordinator says they paid but the status here
                hasn&rsquo;t caught up, or if some delegates never received
                their email. It re-reads the payment from Razorpay and retries
                any delegate still missing a registration.
              </p>
            </CardContent>
          </Card>
                ...
                <div className="p-3 rounded bg-emerald-50 border border-emerald-200 text-sm">
                  ...
                  {group.reviewedAt && (
                    <div className="text-xs text-emerald-700">
                      {format(new Date(group.reviewedAt), "d MMM yyyy, HH:mm")}
                    </div>
                  )}
                </div>
              )}
              {group.status === "rejected" && (
                <div className="p-3 rounded bg-red-50 border border-red-200 text-sm">
                  ...
                  <p className="mt-2 text-xs text-red-700">
                    Remember to process the refund via the Razorpay dashboard.
                  </p>
                </div>
              )}
              {group.status === "submitted" && (
                <p className="text-sm text-[var(--muted-text)]">
                  Awaiting payment — nothing to review yet.
                </p>
              )}
              {group.status === "payment_review" && (
                <p className="text-sm text-[var(--muted-text)]">
                  Legacy payment-review record. New group registrations are
