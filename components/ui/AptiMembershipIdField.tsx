 ...
 */
export default function AptiMembershipIdField({
  registerProps,
  error,
  helperText,
}: AptiMembershipIdFieldProps) {
  ...
  async function checkMembership(memberId: string) {
    ...
    try {
      const res = await fetch(
        `/api/apti-members/verify?memberId=${encodeURIComponent(trimmed)}`,
      );
    ...
    <div>
      ...
      {status === "invalid" && (
        <p className="mt-1 text-xs text-red-600">
          Membership ID not found in the APTI registry.
        </p>
      )}
